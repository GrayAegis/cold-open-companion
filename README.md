...
- **A read-only tracker board.** Rebuilt from the chat itself, not the preset: the latest values per character and per thread, whatever happened to them in between. Hover a number for its history — `6 → 7 → 5 → 6` — and anything that has moved on nearly every appearance is flagged amber, which answers "are these drifting?" directly. It also checks in code what the preset can only ask for in prose: a fuse reading `7/6`, more than two fused threads, more than four live threads, more than six bond lines, a meter outside 0-10. Nothing is written back — the preset stays the single source of truth, and the board disappears when a chat has no tracker lines.
